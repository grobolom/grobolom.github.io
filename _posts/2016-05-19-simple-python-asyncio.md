...
def our_process(queue):
    asyncio.async(something_slow(queue))
...
```python
@asyncio.coroutine
def something_slow(queue):
    yield from asyncio.sleep(10)
    yield from queue.put('something slow is done')
```
...
def reader(queue):
    while True:
        loop = asyncio.get_event_loop()
        future = loop.run_in_executor(None, get_input)
...
That's everything. Hopefully that gives you a better illustration of what happens when we have multiple coroutines running in infinite loops, as well as how they communicate. If you need more examples, I highly recommend looking at [Mionar's chat client](https://github.com/Mionar/aiosimplechat/blob/master/aiosimplechat/client.py) as a similar but more complete example. Also feel free to leave comments or questions below.
